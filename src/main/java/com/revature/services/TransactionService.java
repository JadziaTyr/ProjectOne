...
public class TransactionService
{
	...
	public boolean fetchReimbursementList(List<Reimbursement> list)
	{
		try (Connection conn = DriverManager.getConnection(DAO.URL, DAO.USERNAME, DAO.PASSWORD);)
		{
			...
			dao = new UserDAO(dao.getSavepoint());
			for (int i = 0; i < list.size(); i++)
			{
				if (!(((UserDAO) dao).retrievePersonById(list.get(i).getRequester(), conn)
						&& ((UserDAO) dao).retrievePersonById(list.get(i).getApprover(), conn)))
				{
					dao.rollback(conn);
					return false;
				}
